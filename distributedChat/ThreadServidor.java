...
public class ThreadServidor extends Thread{
    ...
    public void run() {
		try {
	        ...
	        while (!cerrarSesion) {
	            String comandoCliente = in.readUTF();
	...
	private void listarSalas() {
		...
		if(salasAImprimir.size() > 0)
			for(Sala sala : salasAImprimir) mensaje+=sala.toString() + "\n";
		else mensaje = "\nNo available rooms\n";
		try {
	...
	private void crearSala(String[] tablaComandoCliente) {
		...
		try {
			...
			for(Sala sala : new ArrayList<>(salas.values())) {
				if(sala.getId().equals(tablaComandoCliente[1])) {
					out.writeUTF("SALA_NAME_OCCUPIED");
					return;
				}
			}
			//Si la longitud es 4, significa que se ha enviado una contraseña
			if(tablaComandoCliente.length == 4) password = tablaComandoCliente[3].strip();
			agregarSala(new Sala(tablaComandoCliente[1], 
					tablaComandoCliente[2].strip().equalsIgnoreCase("true"), password));
			out.writeUTF("SALA_CREATED");
		} catch (Exception e) {
			try {
				out.writeUTF("ERROR_CREATING_SALA");
	...
	private void selectNickname(DataInputStream in, DataOutputStream out, String[] tablaComandoCliente) {
		...
		try {
			if(occupied) {
				...
				out.writeUTF("NICKNAME_SET");
				this.nombreCliente = newNickname;
				this.nombresClientes.add(newNickname);
				System.out.println("Client nickname set: " + newNickname);
			}
		} catch (IOException e) {
			try {
				out.writeUTF("ERROR");
	...
	private void enSala(DataInputStream in) {
		System.out.println("Client " + nombreCliente + " connected to room " + salaActual.getId());
		salaActual.enviarMensaje(nombreCliente, " joined the chat");
		while(true) {
			try {
				String mensaje = desencriptar(in.readUTF());
				if(mensaje.equals("LEAVE_ROOM")) {
					System.out.println("Client " + nombreCliente + " leaving room " + salaActual.getId());
				...
				System.out.println("Error at reading message from client " + nombreCliente 
						+ " in room " + salaActual.getId());
				e.printStackTrace();
			}
		}
	}
